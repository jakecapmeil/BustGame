...
// A stand-in for the app's BOARD_SIZES lookup.
const boardFor = (count, size) => ({
...
test('matchmake fields the right roster for the rank', () => {
  const m = matchmake(0, boardFor, () => 0.5);
  assert.equal(m.players.length, 2, 'Woodline is 1v1');
  assert.equal(m.players[0].kind, 'human');
  assert.equal(m.players[1].kind, 'ai');
  assert.equal(m.players[1].difficulty, 'easy');
  assert.equal(m.ratings[0], 0, 'my rating is my trophy count');

  const gold = matchmake(1200, boardFor, () => 0.5);
  assert.equal(gold.players.length, 4, 'Gold is 1v3');
  assert.ok(['hard', 'expert'].includes(gold.players[1].difficulty));
});

test('matchmakeSeeded is deterministic', () => {
  const a = matchmakeSeeded(800, boardFor, 7);
  const b = matchmakeSeeded(800, boardFor, 7);
